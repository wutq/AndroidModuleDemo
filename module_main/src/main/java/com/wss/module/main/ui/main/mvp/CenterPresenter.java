...
public class CenterPresenter extends BasePresenter<CenterModule, ICenterView> {
    ...
    public void getTabList() {
        if (isViewAttached()) {
            ...
            list.add(new MainBlock("观察者按钮", R.drawable.main_icon_13, ObserverButtonActivity.class, "利用按钮对多个输入框监听，如：注册"));
            list.add(new MainBlock("多功能对话框", R.drawable.main_icon_14, DialogActivity.class, "常用对话框，支持输入 单按钮等"));
            list.add(new MainBlock("流式布局", R.drawable.main_icon_15, FlowLayoutActivity.class, "流式布局，从左上角位置开始，自动换行"));
            list.add(new MainBlock("聊天", R.drawable.main_icon_16, IMActivity.class, "聊天布局样式，用于测试多功能列表适配器"));
