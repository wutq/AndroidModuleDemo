...
import com.wss.module.wan.ui.setup.SetupActivity;
...
public class HomePresenter extends BasePresenter<HomeModule, IHomeView> {
    ...
    private void getBlock() {
        if (isViewAttached()) {
            ...
            list.add(new Template("体系", R.drawable.wan_icon_1, SetupActivity.class));
    ...
    private void getWXNumber() {
        if (isViewAttached()) {
            getModule().getWXNumber(getContext(), new OnResultListCallBack<List<WXNumber>>() {
                @Override
